...
où chaque $f_j$ est une **spline** — une fonction lisse estimée à partir des données en minimisant un critère des moindres carrés pénalisé :
$$\sum_{i=1}^{n} \left(y_i - \hat{y}_i\right)^2 + \lambda_j \int \left[f_j''(x)\right]^2 dx$$
La pénalité $\int [f_j''(x)]^2 dx$ mesure la courbure ("wiggliness") de la spline. Le paramètre $\lambda_j$ contrôle le compromis biais-variance : $\lambda_j \to 0$ revient à interpoler les données (surapprentissage) ; $\lambda_j \to \infty$ force la spline à être une droite (sous-apprentissage).
...
- Exploré quatre spécifications de GAM en R (effets spatiaux, saisonnalité évolutive, interaction température-vent, sélection automatique) et comparé leurs AIC et déviances expliquées.
